  ...
  if (
    ...
    this.log = function () {
      if ("undefined" != typeof console) {
        ...
        e.unshift("Reader: (Readability)"), console.log.apply(console, e);
      } else {
        var e;
...
(Readability.prototype = {
  ...
  _concatNodeLists: function () {
    var t = Array.prototype.slice,
      e = t.call(arguments).map(function (e) {
        return t.call(e);
      });
    return Array.prototype.concat.apply([], e);
  },
  _getAllNodesWithTag: function (t, e) {
      ...
      if (
        ...
        if (
          ...
          for (var i = t.children[0], a = 0; a < t.attributes.length; a++)
            i.setAttribute(t.attributes[a].name, t.attributes[a].value);
  ...
  _getArticleTitle: function () {
    ...
    / [\|\-\\\/>»] /.test(t)
      ? ((r = / [\\\/>»] /.test(t)),
        s((t = i.replace(/(.*)[\|\-\\\/>»] .*/gi, "$1"))) < 3 &&
          (t = i.replace(/[^\|\-\\\/>»]*[\|\-\\\/>»](.*)/gi, "$1")))
      : -1 !== t.indexOf(": ")
      ? ((n = this._concatNodeLists(
        ...
        this._someNode(n, function (e) {
          ...
          (s((t = i.substring(i.lastIndexOf(":") + 1))) < 3
            ? (t = i.substring(i.indexOf(":") + 1))
            : 5 < s(i.substr(0, i.indexOf(":"))) && (t = i)))
      : (150 < t.length || t.length < 15) &&
        1 === (n = e.getElementsByTagName("h1")).length &&
        (t = this._getInnerText(n[0]));
    e = s((t = t.trim().replace(this.REGEXPS.normalize, " ")));
    return (t =
      e <= 4 && (!r || e != s(i.replace(/[\|\-\\\/>»]+/g, "")) - 1) ? i : t);
  },
  ...
  _checkByline: function (e, t) {
    return (
      ...
      (void 0 !== e.getAttribute &&
        ((i = e.getAttribute("rel")), (a = e.getAttribute("itemprop"))),
      !(
        !(
          "author" === i ||
          (a && -1 !== a.indexOf("author")) ||
  ...
  _grabArticle: function (t) {
    this.log("**** grabArticle ****");
    var i = this._doc,
      a = null !== t;
    if (!(t = t || this._doc.body))
      return this.log("No body found in document. Abort."), null;
    for (var M = t.innerHTML; ; ) {
      this.log("Starting grabArticle loop");
      var H = this._flagIsActive(this.FLAG_STRIP_UNLIKELYS),
        n = [],
      ...
      for (; r; ) {
        ...
        if (this._isProbablyVisible(r))
          if (
          ...
          else if (this._checkByline(r, s)) r = this._removeAndGetNext(r);
          else if (e && this._headerDuplicatesTitle(r))
            this.log(
              "Removing header: ",
              r.textContent.trim(),
            ...
            if (
              ...
              if (
                (-1 !== this.DEFAULT_TAGS_TO_SCORE.indexOf(r.tagName) &&
                  n.push(r),
                "DIV" === r.tagName)
              ) {
                for (var l, o = null, h = r.firstChild; h; ) {
                  var U = h.nextSibling;
      ...
      for (
                  ...
                  this._forEachNode(e, function (e, t) {
                    e.tagName &&
                      ...
                      (void 0 === e.readability &&
                        (this._initializeNode(e), c.push(e)),
        ...
        var _ = c[g],
          u = _.readability.contentScore * (1 - this._getLinkDensity(_));
        (_.readability.contentScore = u),
          this.log("Candidate:", _, "with score " + u);
        for (var m = 0; m < this._nbTopCandidates; m++) {
          var p = d[m];
          if (!p || u > p.readability.contentScore) {
            d.splice(m, 0, _), d.length > this._nbTopCandidates && d.pop();
            break;
          }
        }
      }
      ...
      if (null === f || "BODY" === f.tagName) {
        for (f = i.createElement("DIV"), N = !0; t.firstChild; )
          this.log("Moving child out:", t.firstChild),
            f.appendChild(t.firstChild);
        t.appendChild(f), this._initializeNode(f);
      } else if (f) {
        for (var E = [], T = 1; T < d.length; T++)
          0.75 <= d[T].readability.contentScore / f.readability.contentScore &&
            E.push(this._getNodeAncestors(d[T]));
        if (3 <= E.length)
          for (y = f.parentNode; "BODY" !== y.tagName; ) {
            for (var b = 0, A = 0; A < E.length && b < 3; A++)
              b += Number(E[A].includes(y));
            if (3 <= b) {
              f = y;
              break;
            }
        ...
        for (
          var y = f.parentNode, v = f.readability.contentScore, W = v / 3;
          "BODY" !== y.tagName;

        )
          if (y.readability) {
            var S = y.readability.contentScore;
            if (S < W) break;
            if (v < S) {
              f = y;
              break;
            }
            (v = y.readability.contentScore), (y = y.parentNode);
          } else y = y.parentNode;
        for (y = f.parentNode; "BODY" != y.tagName && 1 == y.children.length; )
          y = (f = y).parentNode;
        f.readability || this._initializeNode(f);
      }
      for (
        var C = i.createElement("DIV"),
          F =
            (a && (C.id = "readability-content"),
            Math.max(10, 0.2 * f.readability.contentScore)),
          L = (y = f.parentNode).children,
          x = 0,
          I = L.length;
        x < I;
        x++
        ...
        this.log(
          ...
          P,
          P.readability ? "with score " + P.readability.contentScore : ""
        ),
          this.log(
            "Sibling has score",
            P.readability ? P.readability.contentScore : "Unknown"
          ),
          P === f
            ? (w = !0)
            : ((D = 0),
              P.className === f.className &&
                "" !== f.className &&
                (D += 0.2 * f.readability.contentScore),
              P.readability && P.readability.contentScore + D >= F
                ? (w = !0)
                : "P" === P.nodeName &&
                  ((D = this._getLinkDensity(P)),
                  ((80 < (O = (R = this._getInnerText(P)).length) &&
                    D < 0.25) ||
                    (O < 80 &&
                      0 < O &&
                      0 === D &&
                    ...
                    (w = !0))),
          w &&
            (this.log("Appending node:", P),
            -1 === this.ALTER_TO_DIV_EXCEPTIONS.indexOf(P.nodeName) &&
              (this.log("Altering sibling:", P, "to div."),
              (P = this._setNodeTag(P, "DIV"))),
            C.appendChild(P),
            (L = y.children),
            --x,
            --I);
      ...
      if (
        (this._debug && this.log("Article content pre-prep: " + C.innerHTML),
        this._prepArticle(C),
        this._debug && this.log("Article content post-prep: " + C.innerHTML),
        N)
      )
        (f.id = "readability-page-1"), (f.className = "page");
      else {
        var B = i.createElement("DIV");
        for (B.id = "readability-page-1", B.className = "page"; C.firstChild; )
          B.appendChild(C.firstChild);
        C.appendChild(B);
      }
      this._debug && this.log("Article content after paging: " + C.innerHTML);
      var N = !0,
        G = this._getInnerText(C, !0).length;
      if (G < this._charThreshold)
        if (
          ...
          this._removeFlag(this.FLAG_STRIP_UNLIKELYS),
            this._attempts.push({ articleContent: C, textLength: G });
          ...
          if (
            ...
            return null;
          (C = this._attempts[0].articleContent), (N = !0);
        }
      if (N)
        return (
          (G = [y, f].concat(this._getNodeAncestors(y))),
  ...
  _getJSONLD: function (e) {
    ...
    return (
      this._forEachNode(e, function (e) {
        if (!s && "application/ld+json" === e.getAttribute("type"))
          try {
            var t,
              i,
              a,
              n = e.textContent.replace(/^\s*<!\[CDATA\[|\]\]>\s*$/g, ""),
              r = JSON.parse(n);
            return r["@context"] &&
              r["@context"].match(/^https?\:\/\/schema\.org$/)
              ? (r =
                  !r["@type"] && Array.isArray(r["@graph"])
                    ? r["@graph"].find(function (e) {
                        return (e["@type"] || "").match(
                    ...
                    : r) &&
                r["@type"] &&
                r["@type"].match(this.REGEXPS.jsonLdArticleTypes)
                ? ((s = {}),
                  "string" == typeof r.name &&
                  "string" == typeof r.headline &&
                  r.name !== r.headline
                    ? ((t = this._getArticleTitle()),
                      (i = 0.75 < this._textSimilarity(r.name, t)),
                      (a = 0.75 < this._textSimilarity(r.headline, t)),
                      (s.title = a && !i ? r.headline : r.name))
                    : "string" == typeof r.name
                    ? (s.title = r.name.trim())
                    : "string" == typeof r.headline &&
                      (s.title = r.headline.trim()),
                  r.author &&
                    ("string" == typeof r.author.name
                      ? (s.byline = r.author.name.trim())
                      : Array.isArray(r.author) &&
                        r.author[0] &&
                        "string" == typeof r.author[0].name &&
                        (s.byline = r.author
                          .filter(function (e) {
                  ...
                  "string" == typeof r.description &&
                    (s.excerpt = r.description.trim()),
                  void (
                    r.publisher &&
                    "string" == typeof r.publisher.name &&
                    (s.siteName = r.publisher.name.trim())
                  ))
                : void 0
              : void 0;
          } catch (e) {
    ...
    return (
      this._forEachNode(i, function (e) {
        ...
        e &&
          ...
          n &&
            (t = n.match(s)) &&
            ((i = t[0].toLowerCase().replace(/\s/g, "")), (r[i] = e.trim())),
          !t &&
            a &&
            l.test(a) &&
            ((i = a),
      ...
      (t.byline =
        e.byline || r["dc:creator"] || r["dcterm:creator"] || r.author),
      (t.excerpt =
        e.excerpt ||
        r["dc:description"] ||
  ...
  _unwrapNoscriptImages: function (o) {
    ...
    this._forEachNode(e, function (e) {
      ...
      if (((t.innerHTML = e.innerHTML), this._isSingleImage(t))) {
        ...
        if (i && this._isSingleImage(i)) {
          for (
            ...
            "" === l.value ||
              ...
              n.getAttribute(l.name) === l.value ||
              ((s = l.name),
              n.hasAttribute(s) && (s = "data-old-" + s),
              n.setAttribute(s, l.value));
          }
  ...
  _getRowAndColumnCount: function (e) {
    for (
      var t = 0, i = 0, a = e.getElementsByTagName("tr"), n = 0;
      n < a.length;
      n++
    ) {
      var r = a[n].getAttribute("rowspan") || 0;
      t += (r = r && parseInt(r, 10)) || 1;
      for (
        var s = 0, l = a[n].getElementsByTagName("td"), o = 0;
        o < l.length;
        o++
      ) {
        var h = l[o].getAttribute("colspan") || 0;
        s += (h = h && parseInt(h, 10)) || 1;
      }
      i = Math.max(i, s);
    }
  ...
  _markDataTables: function (e) {
    for (var t = e.getElementsByTagName("table"), i = 0; i < t.length; i++) {
      ...
      "presentation" == n.getAttribute("role")
        ? (n._readabilityDataTable = !1)
        : "0" == n.getAttribute("datatable")
        ? (n._readabilityDataTable = !1)
        : n.getAttribute("summary") ||
          ((a = n.getElementsByTagName("caption")[0]) &&
            0 < a.childNodes.length)
        ? (n._readabilityDataTable = !0)
        : ["col", "colgroup", "tfoot", "thead", "th"].some(function (e) {
            return !!n.getElementsByTagName(e)[0];
          })
        ? (this.log("Data table because found data-y descendant"),
          (n._readabilityDataTable = !0))
        : n.getElementsByTagName("table")[0]
        ? (n._readabilityDataTable = !1)
        : 10 <= (a = this._getRowAndColumnCount(n)).rows || 4 < a.columns
        ? (n._readabilityDataTable = !0)
        : (n._readabilityDataTable = 10 < a.rows * a.columns);
    }
  },
      ...
      function (e) {
        if (e.src && this.REGEXPS.b64DataUrl.test(e.src)) {
          if ("image/svg+xml" === this.REGEXPS.b64DataUrl.exec(e.src)[1])
            return;
          for (var t, i = !1, a = 0; a < e.attributes.length; a++) {
        ...
        if (
          ...
          for (var r, s, l = 0; l < e.attributes.length; l++)
            "src" !== (n = e.attributes[l]).name &&
              "srcset" !== n.name &&
              "alt" !== n.name &&
              ((r = null),
              ...
              r &&
                ("IMG" === e.tagName || "PICTURE" === e.tagName
                  ...
                  : "FIGURE" !== e.tagName ||
                    ...
                    ((s = this._doc.createElement("img")).setAttribute(
                      r,
                      n.value
                    ),
                    e.appendChild(s))));
  ...
  _cleanConditionally: function (e, N) {
    this._flagIsActive(this.FLAG_CLEAN_CONDITIONALLY) &&
      this._removeNodes(this._getAllNodesWithTag(e, [N]), function (e) {
        function t(e) {
        ...
        if (
          (a ||
            ...
            this._forEachNode(n, (e) => (i += this._getInnerText(e).length)),
        ...
        if (this._getCharCount(e, ",") < 10) {
          for (
            var r = e.getElementsByTagName("p").length,
              s = e.getElementsByTagName("img").length,
              l = e.getElementsByTagName("li").length - 100,
              o = e.getElementsByTagName("input").length,
              h = this._getTextDensity(e, ["h1", "h2", "h3", "h4", "h5", "h6"]),
              c = 0,
              d = this._getAllNodesWithTag(e, ["object", "embed", "iframe"]),
              g = 0;
            g < d.length;
            g++
          ) {
            for (var _ = 0; _ < d[g].attributes.length; _++)
              if (this._allowedVideoRegex.test(d[g].attributes[_].value))
                return !1;
            if (
              "object" === d[g].tagName &&
              this._allowedVideoRegex.test(d[g].innerHTML)
            )
              return !1;
            c++;
          }
          var u = this._getLinkDensity(e),
            ...
            p =
              (1 < s && r / s < 0.5 && !this._hasAncestorTag(e, "figure")) ||
              (!a && r < l) ||
              o > Math.floor(r / 3) ||
              (!a &&
                h < 0.9 &&
          ...
          if (a && p) {
            for (var f = 0; f < e.children.length; f++)
              if (1 < e.children[f].children.length) return p;
  ...
  parse: function () {
    ...
    var e = this._disableJSONLD ? {} : this._getJSONLD(this._doc),
      ...
      t = ((this._articleTitle = e.title), this._grabArticle());
    if (!t) return null;
    this.log("Grabbed: " + t.innerHTML),
      this._postProcessContent(t),
      e.excerpt ||
        (0 < (i = t.getElementsByTagName("p")).length &&
